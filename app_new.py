...
def create_welcome_overview():
    ...
    for ds_id in sorted(datasets.keys()):
        ...
        rows.append(html.Tr([
            html.Td(ds_id, className="td-bold"),
    ...
    overview = html.Div(
        ...
        children=[
            html.H4("Welcome to Shedding Hub Dashboard"),
            html.P("Click 'Create New Tab' to start exploring the data."),
...
def create_description_card():
    ...
    return html.Div(
        ...
        children=[
            ...
            html.Div(
                id="intro",
                children="Explore viral shedding patterns across different biomarkers, specimens, and time courses.",
            ),
        ],
    )
...
def create_control_card():
    """Create the control panel with filters and tab management"""
    return html.Div(
        id="control-card",
        children=[
            ...
            html.Div(
                children=[
                    ...
                    dcc.Dropdown(
                        id="biomarker-select",
                        options=[{"label": bm, "value": bm} for bm in list_biomarker],
                        value=None,
                    ),
                    html.Br(),
                    ...
                    dcc.Dropdown(
                        id="specimen-select",
                        options=[{"label": spec, "value": spec} for spec in list_specimen],
                        value=None,
                    ),
                    html.Br(),

                    html.P("Select Reference Event"),
                    dcc.Dropdown(
                        id="reference-event-select",
                        options=[{"label": evt, "value": evt} for evt in list_reference_events],
                        value=None,
                    ),
                    html.Br(),

                    html.P("Select Value Type"),
                    dcc.RadioItems(
...
def create_tab_content(tab_id, config):
    ...
    if content_type == "statistics":
        ...
        # Plot layout based on study type
        if study_type == "individual":
            return html.Div(
                ...
                children=[
                    ...
                    # Plot container - using html.Img for matplotlib figures
                    html.Div(
...
@callback(
    ...
    Input("reference-event-select", "value"),
    Input("value-type-select", "value"),
    Input("tab-configs", "data"),  # Changed from State to Input to trigger on new tabs
)
def update_tab_plots(plot_types, biomarker, specimen, reference_event, value_type, tab_configs):
    """Update plots in all tabs based on filters and plot type selection"""
...
def update_tab_statistics(biomarker, specimen, reference_event, value_type, tab_configs):
    """Update statistics tables in all statistics tabs"""
    # Get only statistics tabs
    stats_tabs = [(k, v) for k, v in tab_configs.items() if v.get("content_type") == "statistics"]

    if not SHEDDING_HUB_AVAILABLE:
        return [html.Div("Shedding Hub package not available")] * len(stats_tabs)

    contents = []
    for tab_id, config in stats_tabs:
        study_type = config.get("study_type")
        selected_studies = config.get("selected_studies")
...
def generate_individual_statistics(dataset, biomarker, specimen, reference_event, value_type):
    ...
    try:
        ...
        content = html.Div([
            ...
            html.Div([
                pd.DataFrame(shedding_summary).to_html(
                    classes='table table-striped',
                    index=False,
                    float_format=lambda x: f'{x:.2f}'
                )
            ], className="stats-table-wrapper", dangerouslySetInnerHTML={'__html':
                pd.DataFrame(shedding_summary).to_html(
                    classes='table table-striped',
                    index=False,
                    float_format=lambda x: f'{x:.2f}'
...
def generate_comparison_statistics(datasets_list, biomarker, specimen, reference_event, value_type):
    ...
    try:
        ...
        content = html.Div([
            ...
            html.Hr(),
            html.Div([
                pd.DataFrame(comparison).to_html(
                    classes='table table-striped',
                    index=False,
...
@callback(
    ...
    Output("specimen-select", "value", allow_duplicate=True),
    Input("biomarker-select", "value"),
    prevent_initial_call=True
)
def update_specimen_options(selected_biomarker):
    """Update available specimen options based on selected biomarker"""
    if not selected_biomarker:
        options = [{"label": spec, "value": spec} for spec in list_specimen]
        return options, None

    # Filter specimens by selected biomarker
    filtered_specimens = set()
    for dataset in datasets.values():
        for analyte_name, analyte_info in dataset.get('analytes', {}).items():
            if analyte_info.get('biomarker') == selected_biomarker:
                specimen = analyte_info.get('specimen')
                if isinstance(specimen, list):
                    filtered_specimens.update(specimen)
                elif specimen:
                    filtered_specimens.add(specimen)

    options = [{"label": spec, "value": spec} for spec in sorted(filtered_specimens)]
    return options, None


@callback(
    Output("reference-event-select", "options"),
    Output("reference-event-select", "value", allow_duplicate=True),
    Input("biomarker-select", "value"),
    Input("specimen-select", "value"),
    prevent_initial_call=True
)
def update_reference_event_options(selected_biomarker, selected_specimen):
    """Update available reference events based on biomarker and specimen"""
    filtered_events = set()

    for dataset in datasets.values():
        for analyte_name, analyte_info in dataset.get('analytes', {}).items():
            # Check biomarker filter
            if selected_biomarker and analyte_info.get('biomarker') != selected_biomarker:
                continue

            # Check specimen filter
            if selected_specimen:
                specimen = analyte_info.get('specimen')
                if isinstance(specimen, list):
                    if selected_specimen not in specimen:
                        continue
                elif specimen != selected_specimen:
                    continue

            # Add reference event
            ref_event = analyte_info.get('reference_event')
            if ref_event:
                filtered_events.add(ref_event)

    options = [{"label": event, "value": event} for event in sorted(filtered_events)]
    return options, None
...
@callback(
    ...
    Output("reference-event-select", "value"),
    Output("value-type-select", "value"),
    Input("reset-btn", "n_clicks"),
    prevent_initial_call=True
)
def reset_filters(n_clicks):
    """Reset all filters to default values"""
    return None, None, None, "concentration"
